// api/generate.js (Compatible con Node.js 24.x en Vercel)

// Copiamos las estructuras necesarias para que el backend no tire "undefined"
const OBJECTIVES = {
...
export default async function handler(req, res) {
  // Manejo de CORS manual por si las dudas para evitar el Status 0
  ...
  try {
    ...
    // Hardcodeamos temporalmente tu key directo acá para descartar que sea un problema de variables de entorno de Vercel
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: "Error devuelto por Groq", details: errorText });
    }
    ...
    console.error("Crash total de la función:", error);
    return res.status(500).json({ error: "Runtime Error Interno", message: error.message });
  }
}
